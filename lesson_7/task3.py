...
class Cell:
    def __init__(self,bcell):
        self.bcell = int(bcell)
    def __add__(self, other):
        return self.bcell + other.bcell
    def __sub__(self, other):
        result = self.bcell - other.bcell
        return result if result > 0 else 'результат меньше либо равен 0'
    ...
    def make_order(self,row):
        res = ''
        for i in range(int(self.bcell / row)):
            res  += '*' * row + '\n'
        res += '*' * (self.bcell % row) + '\n'
        return res
cell = Cell(24)
